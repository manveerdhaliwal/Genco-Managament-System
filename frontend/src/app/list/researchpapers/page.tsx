...
export default function ResearchPaperPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingIndex !== null) {
      ...
      const updated = [...submittedPapers];
      updated[editingIndex] = formData as PaperData;
      setSubmittedPapers(updated);
      setEditingIndex(null);
    } else {
      // Add new paper
      setSubmittedPapers((prev) => [...prev, formData as PaperData]);
    }
      ...
      <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7"></path>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-3">
            <button
              type="button"
              className={buttonClass("journal")}
              onClick={() => handlePaperSelect("journal")}
            >
              Journal
            </button>
            <button
              type="button"
              className={buttonClass("conference")}
              onClick={() => handlePaperSelect("conference")}
            >
              Conference
            </button>
            <button
              type="button"
              className={buttonClass("workshop")}
              onClick={() => handlePaperSelect("workshop")}
            >
              Workshop
            </button>
          </div>
          ...
          {selectedPaper && (
            <div className="flex flex-col gap-3 mt-3">
              ...
              <input
                type="text"
                name="paperLink"
                placeholder="Link to Paper (DOI / Journal Link)"
            ...
            {submittedPapers.map((paper, index) => (
              ...
              >
                <div>
                  ...
                  {paper.facultyName && (
                    <p className="text-sm">👨‍🏫 Faculty: {paper.facultyName}</p>
                  )}
                </div>
                <button
