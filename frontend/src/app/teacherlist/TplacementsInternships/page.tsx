...
//       try {
//         setLoading(true);
//         const res = await axios.get("http://localhost:5000/api/Placement/allPlacements", {
...
//   //   .filter((p) => selectedYear ? p.yearOfPlacement === selectedYear : true)
...
//         <Link href={`/dashboard/teacher`} className="mb-4 inline-flex items-center justify-center w-10 h-10 bg-indigo-500 text-white rounded-full shadow hover:bg-indigo-600 transition duration-200" aria-label="Go back">
...
//               {years.map((year) => (
//                 <div key={year} onClick={() => setSelectedYear(year)} className="cursor-pointer bg-white shadow-lg rounded-2xl p-6 flex flex-col items-center justify-center hover:shadow-xl transition">
...
//               <button onClick={() => { setSelectedYear(null); setSelectedSection("All"); }} className="bg-gray-200 text-gray-700 px-4 py-2 rounded-xl hover:bg-gray-300 transition">
//                 Back
//               </button>
//             </div>
...
//               <select className="border border-gray-300 rounded-xl px-4 py-2" value={selectedSection} onChange={(e) => setSelectedSection(e.target.value)}>
...
//                         {p.pdfPreview ? (
//                           <button onClick={() => setSelectedPreview(p)} className="bg-indigo-600 text-white px-4 py-1 rounded-xl hover:bg-indigo-700 transition">View</button>
...
//               <button onClick={() => setSelectedPreview(null)} className="absolute top-3 right-3 text-gray-600 hover:text-gray-900 font-bold text-xl">&times;</button>
//               <h2 className="text-xl font-semibold mb-4">{selectedPreview.studentName} - {selectedPreview.title}</h2>
//               {selectedPreview.pdfPreview && <iframe src={selectedPreview.pdfPreview} className="w-full h-96" title="Placement PDF"></iframe>}
//             </div>
//           </div>
//         )}
//       </div>
...
export default function PlacementInternshipPage() {
  ...
  const filteredPlacements = placements
    ...
    .filter((p) => {
      if (!selectedYear) return true;
      return p.studentName && yearMap[selectedYear] ? true : false;
    })
    .sort((a, b) => a.urn.localeCompare(b.urn));
        ...
        {selectedYear && (
          <>
            <div className="flex justify-between items-center">
              <h1 className="text-3xl font-bold text-indigo-700 mb-4">{selectedYear} Year Placements</h1>
              <button
                onClick={() => { setSelectedYear(null); setSelectedSection("All"); }}
