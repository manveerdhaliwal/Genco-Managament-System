...
const saveCertificate = async (req, res) => {
  try {
    ...
    if (existing) {
      existing = await StudentCertificate.findOneAndUpdate(
        { _id: existing._id },
        {
          ...
          eventName,
          date,
          certificateUrl: certificateUrl || existing.certificateUrl, // keep old link if no new file
        },
        { new: true }
      );
      return res.json({
    ...
    const newCert = new StudentCertificate({
      ...
      eventName,
      date,
      certificateUrl, // may be empty if no file uploaded
    });
    ...
    res
      .status(201)
      .json({ success: true, message: "Certificate saved!", data: newCert });
  } catch (error) {
    console.error("Error saving certificate:", error);
    res.status(500).json({ success: false, message: "Server error" });
  }
...
const getMyCertificates = async (req, res) => {
  try {
    const studentId = req.user.id;
    const certs = await StudentCertificate.find({ student: studentId }).sort({
      date: -1,
    });
    res.json({ success: true, data: certs });
  } catch (error) {
...
const getBranchCertificates = async (req, res) => {
  try {
    ...
    if (!students.length)
      return res
        .status(404)
        .json({ success: false, message: "No students found in your branch" });
